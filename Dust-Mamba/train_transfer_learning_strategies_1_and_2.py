...
def get_args():
    ...
    parser.add_argument('--dir_hdfdata', type=str, help='Path to the directory of HDF files')
    parser.add_argument('--dir_csv', type=str, help='Path to the directory of csv file')
    parser.add_argument('--dir_pretrained_model', type=str, help='Path to the pretrained Dust-Mamba')
    parser.add_argument('--train_all_params', type=str, default='1',
                        help='Whether to train all parameters of the model')
...
if __name__ == "__main__":
    ...
    # Dust-Mamba for occurrence detection
    pre_model = Dust_Mamba(18, 2, cfg_oc.optim.bilinear)
    pre_model.load_state_dict(torch.load(args.dir_pretrained_model))

    # Dust-Mamba for intensity detection
    model = Dust_Mamba(18, 7, cfg_oc.optim.bilinear)
    if cuda:
    ...
    # Filter out the weights of the final layer
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'final' not in k}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)

    # print(args.train_all_params)
    if args.train_all_params == 1:
        pass # Default: train all parameters
    else:
        # Freeze the parameters of all layers except the final layer
