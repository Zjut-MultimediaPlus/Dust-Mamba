...
def multiclass_dice_coeff_intensity_levels(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    # Average of Dice coefficient for all classes
    assert input.size() == target.size()
    dice = 0
    dice_list = []
    for channel in range(input.shape[1]):
        dice_t = dice_coeff(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon)
        dice += dice_t
        dice_list.append(dice_t.cpu().numpy())

    return dice / input.shape[1], dice_list
...
def dice_loss(input: Tensor, target: Tensor, multiclass: bool = False):
    ...
    fn = multiclass_dice_coeff_intensity_levels if multiclass else dice_coeff
...
def mIOU_csi_intensity_levels(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    # = CSI
    assert input.size() == target.size()
    iou = 0
    iou_list = []
    for channel in range(input.shape[1]):
        iou_t = IOU_csi(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon,
                        channel=channel + 1)
        iou += iou_t
        iou_list.append(iou_t.cpu().numpy())
    return iou / input.shape[1], iou_list
...
def mPOD_recall_intensity_levels(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    # = Recall
    assert input.size() == target.size()
    pod = 0
    pod_list = []
    for channel in range(input.shape[1]):
        pod_t = POD_recall(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon,
                           channel=channel + 1)
        pod += pod_t
        pod_list.append(pod_t.cpu().numpy())

    return pod / input.shape[1], pod_list
...
def mTAR_precision_intensity_levels(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    # = Precision
    assert input.size() == target.size()
    tar = 0
    tar_list = []
    for channel in range(input.shape[1]):
        tar_t = TAR_precision(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon,
                              channel=channel + 1)
        tar += tar_t
        tar_list.append(tar_t.cpu().numpy())

    return tar / input.shape[1], tar_list
...
def mKappa_intensity_levels(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    assert input.size() == target.size()
    kappa = 0
    kappa_list = []
    for channel in range(input.shape[1]):
        kappa_t = Kappa(input[:, channel, ...], target[:, channel, ...], reduce_batch_first, epsilon,
                        channel=channel + 1)
        kappa += kappa_t
        kappa_list.append(kappa_t.cpu().numpy())

    return kappa / input.shape[1], kappa_list
