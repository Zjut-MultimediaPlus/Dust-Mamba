...
def test_average(model, test_loader, device, epoch, iteration=0):
...
def test_intensity_levels(model, test_loader, device, epoch, iteration=0):
    logger.info("Starting test...")
    model.eval()
    test_batch_num = len(test_loader)
    ...
    if test_batch_num == 0:
        return dice_score, iou, pod, tar, kappa
    return dice_score / test_batch_num, iou / test_batch_num, pod / test_batch_num, tar / test_batch_num, \
           kappa / test_batch_num, dice_list_score, iou_list, pod_list, tar_list, kappa_list
...
def test_visual(model, test_loader, device, epoch, iteration=0):
    ...
    with tqdm(test_loader, total=test_batch_num, desc=f'Testing epoch:{epoch}/{cfg_oc.optim.epochs}', unit='batch',
        ...
        for data, label_true in test_loader:
            ...
            with torch.no_grad():
                ...
                # Convert detections results to numpy arrays
                pred_label = label_pred.argmax(dim=1)[0].cpu().numpy()
                true_label = label_true[0].cpu().numpy()


                # Generate continuous color images
                if hasattr(model, 'out_channels'):
                    save_colored_image(pred_label, "pred_label.png", "Prediction", num_classes=model.out_channels)
                    save_colored_image(true_label, "true_label.png", "Label", num_classes=model.out_channels)
                else:
                    save_colored_image(pred_label, "pred_label.png", "Prediction", num_classes=model.num_classes)
                    save_colored_image(true_label, "true_label.png", "Label", num_classes=model.num_classes)
                ...
                label_pred = torch.from_numpy(pred_label).to(device=device, dtype=torch.long).unsqueeze(0)
                label_true = torch.from_numpy(true_label).to(device=device, dtype=torch.long).unsqueeze(0)
...
def test_net(args, cfg_oc, model, device):
    ...
    if cfg_oc.dataset.num_classes == 2:
        test_score, test_iou, test_pod, test_tar, test_kappa, = test_average(model, test_loader, device, epoch=0)
        logger.info(
        ...
        test_score, test_iou, test_pod, test_tar, test_kappa, test_dice_list, test_iou_list, test_pod_list, test_tar_list, test_kappa_list \
            = test_intensity_levels(model, test_loader, device, epoch=0)
        logger.info(
            f'Test Dice_score: {test_score}, '
            f'Test mIOU_CSI: {test_iou}, '
            f'Test mPOD_recall: {test_pod}, '
    ...
    if cfg_oc.optim.batch_size==1:
        test_visual(model, test_loader, device, epoch=0)
